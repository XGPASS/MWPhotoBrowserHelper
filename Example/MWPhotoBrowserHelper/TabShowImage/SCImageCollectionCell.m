    ...
    if ([image isKindOfClass:[UIImage class]]) {
        ...
        if ([imageString hasPrefix:@"http://"] || [imageString hasPrefix:@"https://"]) {
            ...
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:imageString]
                         placeholderImage:[UIImage imageWithColor:[UIColor lightGrayColor] size:self.imageView.frame.size]];
        } else {
            /// 本地图片
        ...
        self.imageView.image = [UIImage imageWithColor:[UIColor lightGrayColor] size:self.imageView.frame.size];
    }
}
